...
@Controller
@RequestMapping("/itemDailyReport")
public class ItemDailyReportController {
	private static final Logger logger = LoggerFactory.getLogger(ItemDailyReportController.class);
	private static final String DATE_FORMAT_YMD = "yyyy-MM-dd";
	@Autowired
	private ItemDailyReportService itemDailyReportService;
	...
	@RequestMapping(value = "/itemDailyReports.htm")
	@ResponseBody
	public ModelAndView list(HttpServletRequest request) {
		ModelAndView mv = new ModelAndView("itemReport/itemDailyReports");
		...
		List<ItemDailyReport> reports = this.itemDailyReportService.findItemDailyReports(starttime, endtime,
				itemName, param, isnew, isrecommend, onecate, twocate, personUser.getCityId());
		PageInfo<ItemDailyReport> info = new PageInfo<ItemDailyReport>(reports);
		Page<ItemDailyReport> page = new Page<ItemDailyReport>(info.getPageNum(),info.getTotal(),info.getPageSize(),info.getList());
...
//		test(starttime);
	...
	public String exportExcel(HttpServletRequest request,
		...
		try {
			...
			response.setHeader("Content-disposition","attachment;filename=\""
					+ new String((java.net.URLEncoder.encode("商品日报.xls", "UTF-8")).getBytes("UTF-8"),"GB2312") + "\"");
			...
			if (reports != null && !reports.isEmpty()) {
				sheet.mergeCells(0, 0, 21, 0);// 合并标题单元格
				// 指定了字串格式
				...
				Label label = new Label(0, 0, "商品日报", format1);
				sheet.addCell(label);
				...
				sheet.addCell(new Label(19 , 1, "动销率", cellFormat1));
				sheet.addCell(new Label(20 , 1, "周转率", cellFormat1));
				...
				for (int i = 0; i < reports.size(); i++) {
					...
					sheet.addCell(new Number(19, i + 2,turnoverRate== null ? 0 : turnoverRate.doubleValue()));
					BigDecimal shelfSalesRate = reports.get(i).getShelfSalesRate();
					sheet.addCell(new Number(20, i + 2, shelfSalesRate== null ? 0 : shelfSalesRate.doubleValue()));
	...
	private void test(Date starttime){
		...
		List<ItemDailyReport> reports = this.itemDailyReportService.findAllItem();


		Integer orderItemNum = itemDailyReportService.selectOrderItemNum(1, starttime);
		ItemDailyReport  r1= itemDailyReportService.selectShopOrderInfo(1, starttime);
		Integer i1 = itemDailyReportService.selectShopNum(1, starttime);
		ItemDailyReport  r2 = itemDailyReportService.selectOrderInfo(1, starttime);
		Integer i2 = itemDailyReportService.selectOrderShopNum(1, starttime);
		ItemDailyReport  r3 = itemDailyReportService.selectRefundInfo(1, starttime);
		Integer i3 = itemDailyReportService.selectOrderRefundShopNum(1, starttime);
		Integer i4 = itemDailyReportService.selectAllShopNum(1, starttime);
		Integer i5 = itemDailyReportService.selectStockNum(1, starttime);
	}
}
